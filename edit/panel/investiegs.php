...
<form id="form1" name="form1" method="post" action="">
  <p>Nombre de la Investigacion:
    <input type="text" name="nomArticDev" required>
  </p>
  <p>Descripcion de la Investigacion:
    <textarea type="text" name="descticDev" required></textarea>
  </p>

  <p>
<label class="my-1 mr-2" for="inlineFormCustomSelectPref">Ciudad:</label>
    <select name="ubiretCiuMapDev" class="custom-select my-1 mr-sm-2" id="select-location-ciudades" required>
            <optgroup label="       ">
...
<label class="my-1 mr-2" for="select-location-departamentos">Departamento:</label>
    <select name="ubicaterMapDev" class="custom-select my-1 mr-sm-2" id="select-location-departamentos" required>
            <optgroup label="       ">
...
<p>
  ...
  <p>
    <input type="submit" name="submit" id="button" value="Enviar solicitud" />
  </p>
</form>
...
if(isset($_POST["submit"])){
    ...
    if($revisar !== false){
        ...
        $insertar = mysqli_query("INSERT INTO investiger (autor,nomArtic,desctic,ubicaterUrl,ubiretCiuMap,ubiretMap,victer,imagenes,creado) values ('".$_SESSION['usuario']."','".$_POST['nomArticDev']."','".$_POST['descticDev']."','".$_POST['ubicaterUrlDev']."','".$_POST['ubiretCiuMapDev']."','".$_POST['ubicaterMapDev']."','0',$imgContenido',now())");
        // COndicional para verificar la subida del fichero
        if($insertar){
            echo "Gracias " . $_SESSION['usuario'] . ", Pronto nos pondremos en contacto"; 
            echo '<meta http-equiv="refresh" content="1;url=investiegs.php" />';
        }else{
            echo "No pudimos enviar esta solicitud, intentalo mas tarde.";
            echo '<meta http-equiv="refresh" content="1;url=investiegs.php" />';
        } 
        // Sie el usuario no selecciona ninguna imagen
    }else{
        echo "Por favor agrega el archivo de la investigacion.";
    }
}
?>
</body>
